...
- **daily 概览改版**（2026-06-18）：紧凑卡片 = 品牌行（Token Tracker 红 + agent 暗红）+ 红分割线 + `Last 12 months` 周期标题（带日期范围）+ 过去一年三行分析：橙 Tokens/Cost/Sessions/Avg-Cost/Active-Days、蓝 Peak/Longest Session/Max Streak、粉 Busiest/Top Project/Top Model/Active Hour，与 weekly 顶部样式统一。数据筛过去一年；最长会话过滤跨度 > 一周的异常；Active Hour = 过去一月按本地时区聚合 token、取活跃小时区间（支持跨午夜）；接入 sessions（最长会话）+ entries（活跃时段）。配色走 Catppuccin 语义槽位、具体色值见 `theme.py`。
