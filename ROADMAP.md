...
- **daily 概览改版**（2026-06-18）：紧凑卡片 = 品牌行（Token Tracker 红 + agent 暗红）+ 红分割线 + `Last 12 months` 周期标题 + 过去一年汇总两行（橙 Tokens/Cost/Days、蓝 Sessions/Msgs，空格分隔），与 weekly 顶部样式统一。数据筛过去一年（365 天，与热力图范围一致）。配色走 Catppuccin 语义槽位、具体色值见 `theme.py`。
