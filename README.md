...
| 行 | 字段 | 说明 |
|----|------|------|
| 1 | `项目名(分支 +12 -3)` | 当前项目目录 + Git 分支（未提交修改标 `*`），括号内附工作区相对 HEAD 的增删行数 |
...
| 4 | `TPS: 60 tokens/s` | 本轮 output token 生成速度（含 thinking；空闲帧保留上次值） |
| 4 | `Code +208 -8` | 本会话 Claude 累计写 / 删的代码行数 |
| 4 | `Repo: github.com` | 代码仓库 host |
